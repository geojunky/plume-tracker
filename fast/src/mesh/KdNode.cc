 ...
 */
     ...
     *--------------------------------------------------------------------------------------
     */
    void KdNode::Expand( float *newCoord )
    {
        if( m_upper == NULL )
        {
            m_upper = new float[DIM];
            memcpy( m_upper, newCoord, sizeof( float )*DIM );

            m_lower = new float[DIM];
            memcpy( m_lower, newCoord, sizeof( float )*DIM );
     ...
     *--------------------------------------------------------------------------------------
     */
    bool KdNode::Intersects( float *up0, float *low0, float *up1, float *low1 )
    {
     ...
     *--------------------------------------------------------------------------------------
     */
    void KdNode::Range( float *upper, float *lower, vector<KdItem*> *result )
    {
     ...
     *--------------------------------------------------------------------------------------
     */
    void KdNode::RangeHelper( float *upper, float *lower, vector<KdItem*> *result )
    {
        if( m_bucket == NULL )
        {
     ...
     *--------------------------------------------------------------------------------------
     */
    bool KdNode::Contains( float *upper, float *lower, float *pnt )
    {
        for( int i=0; i<DIM; i++ )
        {
            if( ( pnt[i] < upper[i] ) &&
