...
using namespace std;
    ...
    void KdTree::Add( float *c, int id )
    {
        ...
        for(int i=0; i<DIM; i++) ki->m_coord[i] = c[i];
        ki->m_id = id;
     ...
     *--------------------------------------------------------------------------------------
     */
    void KdTree::Range( float *upper, float *lower, vector<KdItem*> *result )
    {
        if( m_root )
        {
    ...
    void KdTree::QueryBallPoint(float *pos, float r, vector<float> *distance, vector<int> *id)
    {
            ...
            rit != initialResults.end(); rit++)
        {
            ...
            for(int i=0; i<DIM; i++)
            {
                float diff = (*rit)->m_coord[i] - pos[i];
                dist += diff*diff;
            }
